<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('admin.pages.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-sm rounded-lg p-6 space-y-6">
                @csrf
                ...
                <div>
                    <x-input-label for="slug" :value="__('Slug / URL Path (leave empty to auto-generate)')" />
                    <x-text-input id="slug" name="slug" type="text" class="mt-1 block w-full" :value="old('slug')" placeholder="e.g. catering-services" />
                </div>

                <div>
                    <x-input-label for="content" :value="__('Page Content (HTML supported)')" />
                    <textarea id="content" name="content" rows="12" class="mt-1 block w-full font-mono text-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('content') }}</textarea>
                </div>
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="meta_description" name="meta_description" rows="2" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('meta_description') }}</textarea>
                        </div>
                ...
                <div class="border-t pt-4">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_published" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('is_published', '1') ? 'checked' : '' }}>
                        <span class="ms-2 text-sm font-medium text-gray-700">{{ __('Publish Page Immediately') }}</span>
                    </label>
                </div>
